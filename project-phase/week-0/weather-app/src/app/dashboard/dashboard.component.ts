...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this.infoService.getRandomCitiesWeatherInfo().subscribe((response: ListOfCities) => {
      this.cities = response.list.map(oneCity => { return new City(oneCity.id, oneCity.name, oneCity.sys.country, Math.round((oneCity.main.temp)), `../../assets/${oneCity.weather[0].main}.png`) });
